...
from pyproptest.core.shrinker import (
    Shrinkable, binary_search_shrinkable, shrink_element_wise, 
    shrink_membership_wise, shrinkable_array, shrinkable_boolean, 
    shrinkable_float
)
...
class TestShrinkable:
    ...
    def test_with_shrinks(self):
        ...
        shr = Shrinkable(2).with_shrinks(lambda: Stream.many([Shrinkable(0), Shrinkable(1)]))
        assert shr.value == 2
    ...
    def test_map(self):
        ...
        from pyproptest.core.stream import Stream
        shr = Shrinkable(4).with_shrinks(lambda: Stream.many([Shrinkable(0), Shrinkable(2), Shrinkable(3)]))
    ...
    def test_retrieve(self):
        ...
        from pyproptest.core.stream import Stream
        shr = Shrinkable(4).with_shrinks(lambda: Stream.many([
            Shrinkable(0),
...
class TestElementWiseShrinking:
    ...
    def test_shrink_element_wise_single_element(self):
        ...
        from pyproptest.core.stream import Stream
        elem = Shrinkable(5).with_shrinks(lambda: Stream.many([Shrinkable(3), Shrinkable(4)]))
        shr = Shrinkable([elem])
        result = shrink_element_wise(shr, 0, 0)
...
class TestShrinkableArray:
    ...
    def test_shrinkable_array_membership_only(self):
        ...
        elem1 = Shrinkable(5)
        elem2 = Shrinkable(7)
        result = shrinkable_array([elem1, elem2], 0, membership_wise=True, element_wise=False)
        assert result.value == [5, 7]
        # Should have shrinks with fewer elements
        shrinks_list = result.shrinks().to_list()
        assert len(shrinks_list) > 0
    ...
    def test_shrinkable_array_element_wise_only(self):
        ...
        elem1 = Shrinkable(5).with_shrinks(lambda: Stream.one(Shrinkable(3)))
        elem2 = Shrinkable(7).with_shrinks(lambda: Stream.one(Shrinkable(6)))
        result = shrinkable_array([elem1, elem2], 2, membership_wise=False, element_wise=True)
        assert result.value == [5, 7]
    ...
    def test_shrinkable_array_both(self):
        ...
        elem1 = Shrinkable(5).with_shrinks(lambda: Stream.one(Shrinkable(3)))
        elem2 = Shrinkable(7).with_shrinks(lambda: Stream.one(Shrinkable(6)))
        result = shrinkable_array([elem1, elem2], 0, membership_wise=True, element_wise=True)
        assert result.value == [5, 7]
