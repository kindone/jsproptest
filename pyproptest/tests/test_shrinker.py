...
import pytest
from pyproptest import Gen, run_for_all, PropertyTestError, Shrinkable
from pyproptest.core.shrinker import (
    IntegerShrinker, StringShrinker, ListShrinker, DictShrinker,
    shrink_to_minimal
)
import random
...
class TestShrinker:
    ...
    def test_shrinkable_preserves_value_and_shrinks(self):
        ...
        shrinkable_with_shrinks = Shrinkable(42, lambda: Stream.many([Shrinkable(21), Shrinkable(0)]))
    ...
    def test_shrinking_with_dict_generator(self):
        ...
        with pytest.raises(PropertyTestError) as exc_info:
            run_for_all(
                test_failing_property,
                Gen.dict(Gen.str(min_length=1, max_length=2), Gen.int(min_value=0, max_value=10),
