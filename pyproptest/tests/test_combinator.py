...
import pytest
from pyproptest import Gen, run_for_all, PropertyTestError, for_all, integers
import random
...
class TestCombinators:
    ...
    def test_multiple_transformations_chained(self):
        ...
        rng = random.Random(42)
        chained_gen = (Gen.int(min_value=1, max_value=100)
                      .map(lambda x: x * 2)
    ...
    def test_combinator_performance_with_large_runs(self):
        ...
        @for_all(integers(min_value=1, max_value=100).map(lambda x: f"Number: {x}"), num_runs=1000)
        def test_property(self, x: str):
