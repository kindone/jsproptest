...
import pytest
from pyproptest import Gen, run_for_all, PropertyTestError
import random
...
class TestGeneratorInterface:
    ...
    def test_flat_map_chaining_works(self):
        ...
        chained_gen = int_gen.flat_map(
            lambda i: Gen.int(min_value=i, max_value=i + 10)
        )
    ...
    def test_chain_preserves_original_value(self):
        ...
        chained_gen = int_gen.flat_map(
            lambda i: Gen.just((i, i + 10))  # Return tuple
        )
    ...
    def test_multiple_transformations(self):
        ...
        gen = (Gen.int(min_value=1, max_value=100)
    ...
    def test_nested_flat_map(self):
        ...
        gen = (Gen.int(min_value=1, max_value=5)
               .flat_map(lambda length: Gen.list(Gen.int(min_value=0, max_value=10), 
