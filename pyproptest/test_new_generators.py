...
import pytest
from pyproptest.core.generator import Gen, SetGenerator, UnicodeStringGenerator, LazyGenerator, ConstructGenerator, ChainTupleGenerator
...
class TestSetGenerator:
    ...
    def test_set_generator_unique_elements(self):
        ...
        shrinkable = gen.generate(rng)
        assert len(shrinkable.value) <= 3  # Can't have more unique elements than range size
...
class TestChainTupleGenerator:
    ...
    def test_chain_tuple_generator_nested(self):
        ...
        pair_gen = Gen.chain_tuple(single_gen, lambda single: Gen.int(0, max(1, single[0] - 2)))
        triple_gen = Gen.chain_tuple(pair_gen, lambda pair: Gen.int(0, max(1, pair[1] - 1)))
        rng = __import__('random').Random(42)
