...
from typing import Any, Callable, Generic, TypeVar, Union, Optional, List, Dict, Set
...
class FilteredGenerator(Generator[T]):
    ...
    def __init__(self, generator: Generator[T], predicate: Callable[[T], bool], max_attempts: int = 100):
        self.generator = generator
    ...
    def generate(self, rng: Random) -> Shrinkable[T]:
        for _ in range(self.max_attempts):
            ...
            if self.predicate(shrinkable.value):
                return shrinkable
        raise ValueError(f"Could not generate value satisfying predicate after {self.max_attempts} attempts")
...
class Gen:
    ...
    @staticmethod
    def str(min_length: int = 0, max_length: int = 20, charset: str = "abcdefghijklmnopqrstuvwxyz") -> Generator[str]:
    ...
    @staticmethod
    def float(min_value: float = -1000.0, max_value: float = 1000.0) -> Generator[float]:
        """Generate random floats in the specified range."""
        return FloatGenerator(min_value, max_value)
    
    @staticmethod
    def list(element_generator: Generator[T], min_length: int = 0, max_length: int = 10) -> Generator[List[T]]:
        """Generate random lists of elements from the given generator."""
        return ListGenerator(element_generator, min_length, max_length)
    
    @staticmethod
    def dict(key_generator: Generator[T], value_generator: Generator[U], 
             min_size: int = 0, max_size: int = 10) -> Generator[Dict[T, U]]:
    ...
    @staticmethod
    def set(element_generator: Generator[T], min_size: int = 0, max_size: int = 10) -> Generator[Set[T]]:
    ...
    @staticmethod
    def chain_tuple(tuple_gen: Generator[tuple], gen_factory: Callable[[tuple], Generator[Any]]) -> Generator[tuple]:
...
class ListGenerator(Generator[List[T]]):
    ...
    def __init__(self, element_generator: Generator[T], min_length: int, max_length: int):
    ...
    def _generate_shrinks(self, elements: List[Shrinkable[T]]) -> List[Shrinkable[List[T]]]:
        ...
        # Shorter lists
        if len(elements) > 1:
            shrinks.append(Shrinkable([elem.value for elem in elements[:-1]]))  # Remove last
            shrinks.append(Shrinkable([elem.value for elem in elements[1:]]))    # Remove first
...
class DictGenerator(Generator[Dict[T, U]]):
    ...
    def __init__(self, key_generator: Generator[T], value_generator: Generator[U], 
    ...
    def _generate_shrinks(self, items: List[tuple[Shrinkable[T], Shrinkable[U]]]) -> List[Shrinkable[Dict[T, U]]]:
        ...
        if len(items) > 1:
            shrinks.append(Shrinkable({key.value: value.value for key, value in items[:-1]}))
            shrinks.append(Shrinkable({key.value: value.value for key, value in items[1:]}))
        ...
        for i, (key_shrinkable, value_shrinkable) in enumerate(items):
            for shrunk_value in value_shrinkable.shrinks().to_list():
                ...
                shrinks.append(Shrinkable({key.value: value.value for key, value in new_items}))
...
class SetGenerator(Generator[Set[T]]):
    ...
    def _generate_shrinks(self, elements: List[Shrinkable[T]]) -> List[Shrinkable[Set[T]]]:
...
class ChainTupleGenerator(Generator[tuple]):
    ...
    def __init__(self, tuple_gen: Generator[tuple], gen_factory: Callable[[tuple], Generator[Any]]):
    ...
    def generate(self, rng: Random) -> Shrinkable[tuple]:
        ...
        for shrunk_tuple in tuple_shrinkable.shrinks().to_list():
            ...
            shrinks.append(Shrinkable(shrunk_tuple.value + (new_dependent_shrinkable.value,)))
        
        # Shrinks from dependent value generation
        for shrunk_dependent in dependent_shrinkable.shrinks().to_list():
            shrinks.append(Shrinkable(tuple_shrinkable.value + (shrunk_dependent.value,)))
