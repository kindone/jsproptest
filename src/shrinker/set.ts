...
export function shrinkableSet<T>(set:Set<Shrinkable<T>>, minSize:number):Shrinkable<Set<T>> {
    const size = set.size
    ...
    return shrinkableArr.transform(theArr => new Set(theArr.map(shr => shr.value)))
}