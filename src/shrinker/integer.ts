...
function genpos(min: number, max: number): Stream<Shrinkable<number>> {
    const mid =
        ...
        Math.floor(max / 2) +
        (min % 2 !== 0 && max % 2 !== 0 ? 1 : 0);
    if (min + 1 >= max) return Stream.empty();
    else if (min + 2 >= max) return Stream.one(new Shrinkable<number>(mid));
    else
        return Stream.one(
            new Shrinkable<number>(mid).with(() => genpos(min, mid))
        ).concat(genpos(mid, max));
}
...
export function binarySearchShrinkable(value: number): Shrinkable<number> {
    return new Shrinkable<number>(value).with(() => {
        if (value === 0) return Stream.empty();
            ...
            return Stream.one(new Shrinkable<number>(0)).concat(
                genpos(0, value)
            );
        else
            return Stream.one(new Shrinkable<number>(0)).concat(
