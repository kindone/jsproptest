...
export class Shrinkable<T> {
    constructor(readonly value:T, readonly shrinksGen:() => Stream<Shrinkable<T>> = () => new Stream<Shrinkable<T>>()) {
    }
    ...
    concatStatic(then: () => Stream<Shrinkable<T>>):Shrinkable<T> {
        return this.with(() =>
            this.shrinks().transform(shr => shr.concatStatic(then)).concat(then())
        )
    }
    ...
    concat(then:(_:Shrinkable<T>) => Stream<Shrinkable<T>>):Shrinkable<T> {
        const self = this
        return this.with(() =>
            this.shrinks().transform(shr => shr.concat(then)).concat(then(self))
        )
    }
    ...
    andThenStatic(then: () => Stream<Shrinkable<T>>):Shrinkable<T> {
        if(this.shrinks().isEmpty())
        {
            ...
            return this.with(() =>
                this.shrinks().transform((shr) =>
                    shr.andThenStatic(then)
                )
            )
        }
    }
    ...
    transform<U>(transformer:(_:T) => U):Shrinkable<U> {
        ...
        return shrinkable.with(() => this.shrinksGen().transform(shr => shr.transform<U>(transformer)))
    }
