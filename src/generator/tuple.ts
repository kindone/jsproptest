...
export function TupleGen<GeneratorTs extends Generator<unknown>[]>(
    ...elemGens: GeneratorTs
): Generator<GenValueTypes<GeneratorTs>> {
    type Ts = GenValueTypes<GeneratorTs>;
    return new Arbitrary<Ts>((rand: Random) => {
        const shrinkables: any[] = elemGens.map(elemGen =>
            elemGen.generate(rand)
        ) as ShrsFromGens<GeneratorTs>;
        return shrinkableTuple(...shrinkables) as Shrinkable<
            GenValueTypes<GeneratorTs>
        >;
    });
}
