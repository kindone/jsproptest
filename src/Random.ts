...
export class Random {
    ...
    nextLong(min:number = -2147483648*1000000, max:number = 2147483648*1000000):number {
    ...
    inRange(fromInclusive:number, toExclusive:number):number {
        return fromInclusive + (Math.abs(this.nextLong()) % (toExclusive-fromInclusive))
    }
    ...
    intInRange(fromInclusive:number, toExclusive:number):number {
        return fromInclusive + (Math.abs(this.nextInt()) % (toExclusive-fromInclusive))
    }
}