...
export class Either<Left, Right> {
    ...
    getLeft(): Left {
        if (!this.isLeft()) throw new Error('left not valid')
        return this.left.get()
    }

    getRight(): Right {
        if (!this.isRight()) throw new Error('right not valid')
    ...
    filterOrElse<NewLeft>(fn: (right: Right) => boolean, left: NewLeft): Either<NewLeft, Right> {
        if (this.isRight() && fn(this.getRight())) return new Either(None(), this.right)
        else return new Either(Some(left), None())
    }
    ...
    filterOrElseLeft<NewRight>(fn: (left: Left) => boolean, right: NewRight): Either<Left, NewRight> {
        if (this.isLeft() && fn(this.getLeft())) return new Either(this.left, None())
        else return new Either(None(), Some(right))
    }
}
