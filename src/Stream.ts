class Iterator<T> {
    ...
    hasNext():boolean { return !this.stream.isEmpty() }

    next():T {
        if(!this.hasNext())
            throw 'iterator has no more next'
...
export class Stream<T> {
    ...
    iterator() { return new Iterator<T>(this)}
    ...
    filter(criteria:(_:T) => boolean):Stream<T> {
        if(this.isEmpty())
            ...
            for(var itr = this.iterator(); itr.hasNext();) {
                ...
                if(criteria(value)) {
                    return new Stream<T>(value, () => itr.stream.filter(criteria))
                }
            }
            return Stream.empty<T>()
        }
