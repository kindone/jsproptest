...
- **Implementation**: `FloatGenConfig` interface in `src/generator/floating.ts`; uses `oneOf` + `weightedGen` for special values; validation for individual probs and sum ≤ 1.0.
...
- **JS API (proposed)**: `new Property(...).setMaxDurationMs(5000)` or `forAll(..., { maxDurationMs: 5000 })`
...
- **[x] Floating point finite-only generation** — `FloatingGen()` generates `[0, 1)` finite values with shrinking towards 0
